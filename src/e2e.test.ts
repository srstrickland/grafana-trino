...
// the secure SOCKS proxy (no direct network route from Grafana) - see
// docs/pdc-testing.md. Proves the proxy toggle actually routes traffic
// rather than just accepting the setting, since a query can only succeed
// here if it truly went through the proxy.
const PDC_PRIVATE_TRINO_URL = process.env.PDC_PRIVATE_TRINO_URL ?? 'http://trino-private:8080';

async function setupDataSourceWithSecureSocksProxy(page: Page) {
...
test('test with secure socks proxy (PDC)', async ({ page }) => {
    await login(page);
    await goToTrinoSettings(page);
    await setupDataSourceWithSecureSocksProxy(page);
